...
void initialize() {
    ...
    // (v3)------(v2)
    figures::Cube cube;
    scene.addFigure(std::make_shared<figures::Cube>(cube));

    std::vector<Vertex> vertices;
    std::vector<uint32_t> indices;

    figures::Cube::createCube(vertices, indices);
...
void update(double time) {
    ...
    ImGui::SliderFloat("Rotation Speed", &rotation_speed, 0.01f, 2.0f * M_PI);
...
void render(VkCommandBuffer cmd, VkFramebuffer framebuffer) {
    ...
    // NOTE: ShaderConstant updates, vkCmdXXX expected to be here
    {
        ...
        vkCmdDrawIndexed(cmd, scene.getTotalIndexCount(), 1, 0, 0, 0);
    }
